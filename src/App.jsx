...
import { 
  ...
  Sparkles, ArrowRight, Scale, Search, Moon, Sun
} from 'lucide-react';
import { 
  initializeApp 
} from 'firebase/app';
...
// Aktifkan Mod Luar Talian (Offline Firebase Persistence)
try {
...
export default function JomShoppingApp() {
  ...
  // State: Dark Mode (Simpan di Local Storage)
  ...
  useEffect(() => {
    ...
    if (roomFromUrl) {
      const formattedCode = roomFromUrl.toUpperCase();
  ...
  const addItem = async (e) => {
    ...
    const newItem = { id: newItemId, name: newItemName.trim(), category: newItemCategory, price: 0, isBought: false };
  ...
  const addFromMaster = async (masterItem) => {
    ...
    const newItem = { id: Date.now().toString(), name: masterItem.name, category: masterItem.category, price: 0, isBought: false };
  ...
  const handleItemClick = async (item) => {
    if (item.isBought) {
      const newList = roomData.activeList.map(i => i.id === item.id ? { ...i, isBought: false, price: 0 } : i);
  ...
  const confirmItemPrice = async (e) => {
    ...
    const newList = roomData.activeList.map(item => item.id === priceModalItem.id ? { ...item, isBought: true, price: finalPrice } : item);
    await updateRoomData({ activeList: newList });
  ...
  // Helper function untuk benarkan nombor & titik perpuluhan sahaja
  const handleNumberInput = (setter) => (e) => {
    let val = e.target.value.replace(/[^0-9.]/g, '');
    if ((val.match(/\./g) || []).length > 1) return; // elak dua titik perpuluhan
    setter(val);
  };
  ...
  if (!roomCode || !roomData) {
    return (
      <div className={`min-h-screen ${isDarkMode ? 'bg-slate-900' : 'bg-gradient-to-br from-teal-50 to-emerald-100'} flex flex-col items-center justify-center p-4 relative overflow-hidden transition-colors duration-300`} style={{ fontFamily: "'Poppins', sans-serif" }}>
        ...
        {/* KREDIT DI SKRIN LOG MASUK */}
        <div className="absolute bottom-6 left-0 right-0 text-center z-10">
          <p className={`text-[10px] font-bold uppercase tracking-widest ${isDarkMode ? 'text-slate-500' : 'text-teal-700/40'}`}>
            Created by Wan SK
          </p>
        </div>
      </div>
  ...
  // Skrin Utama
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-slate-900' : 'bg-[#F8FAFC]'} pb-28 transition-colors duration-300`} style={{ fontFamily: "'Poppins', sans-serif" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&display=swap');`}</style>
      ...
      <div className="max-w-3xl mx-auto px-4 mt-4 relative z-0">
        ...
        {Object.keys(groupedItems).length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className={`text-lg font-bold mb-1 ${isDarkMode ? 'text-slate-200' : 'text-gray-700'}`}>Senarai Masih Kosong</h3>
            <p className={`text-sm font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>Mula tambah barang keperluan anda di bawah.</p>
          </div>
        ) : (
            ...
            {Object.keys(groupedItems).map(category => (
              <div key={category} className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center gap-2 mb-3 px-1">
                  ...
                  <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${isDarkMode ? 'bg-teal-900/50 text-teal-300' : 'bg-teal-100 text-teal-700'}`}>
                  ...
                  {groupedItems[category].map(item => (
                    <div 
                      ...
                      className={`group rounded-2xl border transition-all duration-300 overflow-hidden ${
                        item.isBought 
                          ? (isDarkMode ? 'bg-slate-800/40 border-slate-700/50 opacity-70' : 'bg-gray-50/80 border-gray-200/60 opacity-80')
                      ...
                      <div className="flex items-center justify-between p-3.5">
                        <div className="flex items-center gap-3.5 flex-1 cursor-pointer" onClick={() => handleItemClick(item)}>
                          <button className="flex-shrink-0 focus:outline-none transform transition-transform active:scale-90">
                            {item.isBought ? (
                              <CheckCircle2 className={`w-6 h-6 ${isDarkMode ? 'text-teal-500 fill-slate-800' : 'text-teal-500 fill-teal-50'}`} />
                            ) : (
                          ...
                          <div className="flex-1">
                            ...
                            {item.isBought && item.price > 0 && (
                              <span className={`inline-block mt-0.5 text-[11px] font-bold px-2 py-0.5 rounded-md ${isDarkMode ? 'bg-teal-900/40 text-teal-400' : 'bg-teal-50 text-teal-600'}`}>
        ...
        {(roomData?.activeList || []).some(i => i.isBought) && (
          <div className="mt-8 mb-4 text-center">
            <button onClick={clearCompleted} className={`inline-flex items-center gap-2 text-xs font-semibold px-5 py-2.5 rounded-full transition-colors ${isDarkMode ? 'bg-teal-900/30 text-teal-400 hover:bg-teal-900/50' : 'bg-teal-50 text-teal-600 hover:bg-teal-100'}`}>
        ...
        <div className={`fixed inset-0 backdrop-blur-sm z-[60] flex items-center justify-center p-4 ${isDarkMode ? 'bg-slate-900/80' : 'bg-slate-900/60'}`}>
          <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-[2rem] w-full max-w-xs overflow-hidden p-6 animate-in zoom-in-95 duration-200 shadow-2xl border ${isDarkMode ? 'border-slate-700' : 'border-transparent'}`}>
            <div className="text-center mb-5">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3 ${isDarkMode ? 'bg-teal-900/50' : 'bg-teal-50'}`}>
                <Tag className={`w-6 h-6 ${isDarkMode ? 'text-teal-400' : 'text-teal-600'}`} />
              </div>
              <h3 className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>{priceModalItem.name}</h3>
              <p className={`text-xs font-medium mt-1 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Sila masukkan harga barang ini</p>
            </div>
